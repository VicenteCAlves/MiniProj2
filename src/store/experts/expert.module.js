    ...
    return new Promise((resolve, reject) => {
      expertService.addExpert(rootState.auth.token, payload).then(
        res => {
          commit(
            SET_MESSAGE,
            `O perito ${res.body.name} foi adicionado com sucesso!`
          );
          resolve(res);
        },
        err => reject(err)
    ...
    return new Promise((resolve, reject) => {
      expertService.editExpert(rootState.auth.token, payload).then(
        res => {
          commit(
            SET_MESSAGE,
            `O perito ${res.body.name} foi atualizado com sucesso!`
          );
          resolve(res);
        },
        err => reject(err)
  ...
  [REMOVE_EXPERT]: ({ commit, rootState }, id) => {
    return new Promise((resolve, reject) => {
      expertService.removeExpert(rootState.auth.token, id).then(
        res => {
          commit(SET_MESSAGE, `O perito foi removido com sucesso!`);
          resolve(res);
        },
        err => reject(err)
      );
